...
- **调试工具 / 小程序**：`wechat_miniprogram/rider_core_temp_debug` 只负责 BLE Central、调试帧解码、记录导出和 OTA 适配，不反向依赖固件源码。
