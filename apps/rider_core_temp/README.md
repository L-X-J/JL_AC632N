...
CORE 温度通知的核心温度字段是有符号百分之一摄氏度；当前实现同时支持 M601 固定贴肤点的合法 Skin Temperature、算法 Core Temperature、Quality & State 和外部心率字段。通过 `30~45°C` 佩戴窗口和 M601 校验的有效样本，在 `WARMING` 阶段先附带 Skin Temperature，核心字段保持 `0x7FFF`；连续 5 个原始样本落在正常 `35~38°C` 区间，或累计 30 个佩戴窗口内有效样本后进入 `STABLE`。当前 AC632N 板级显式选择 `EXPERIMENTAL`：稳定后将多个有效皮肤样本经过中值/EWMA、斜率、滞后和限速模型得到的 Core 候选，与 Skin 字段同时发送到自定义 CORE，供码表记录和现实场景校准；该候选尚未通过参考核心温度验证，不能当作医疗核心体温。核心滞后状态使用 Q8 小数累积，正向和负向变化使用同一条路径，避免皮温下降时被整数舍入吞掉。标准 HTS 和广播只发送 Core 候选，不承载 Skin 字段。完成留出时段验证后才切换 `STRICT`；`CONTACT_PROXY` 仅保留作旧版本对比。Quality 低 4 位反映滤波后的温度信号质量；没有外部心率时，仅心率关联状态表示“支持但未收到信号”。Control Point 只实现协议文档中约定的外部心率输入 `0x13`，其他操作返回“不支持”。核心无效温度编码为 `0x7FFF`。
...
串口中 `ble=0/1/2` 分别表示未启动、广播、已连接；`contact_centi=3650` 和 `skin_centi=3650` 表示滤波后的合法贴肤温度 `36.50°C`，`core_est_centi=3650` 表示实验性算法候选，`publish_mode=3 publish_core_centi=3650` 表示当前板级正在发送该 Core 候选。`warmup=有效样本数/35~38°C样本数` 用于判断 LED2 一直慢闪的原因；估算器在温度状态或新鲜度变化时会打印完整快照。M601 原始采样的 `status=0` 表示通过 CRC 和物理范围；估算器快照的 `state=0/1/2/3/4` 分别表示无设备、未佩戴、预热、稳定、陈旧，`freshness=0/1/2` 分别表示不可用、最新、陈旧。无效核心温度统一打印为 `NA(32767)`，不会把未知字段伪装成 `0°C`。若持续出现 `status=2`，应优先核对 M601 CRC 多项式和 PB7 上拉/时序；若持续出现 `status=1`，应核对 PB7 连线、外部上拉和传感器供电；若持续出现 `status=4`，应确认传感器贴合位置并按实际佩戴曲线校准区间。
...
硬件验收必须至少覆盖：广播服务 UUID 和名称、连接/断连、温度与标准体温 CCCD、Control Point indication（连续写入应返回 busy）、无设备/CRC 错误、`23°C` 脱落、单点尖峰、连续断报/重新佩戴、预热到稳定状态、CORE Flags/皮温字段、EXPERIMENTAL 下 Skin 与算法 Core 同帧、SHADOW 下的核心 `0x7FFF` 和 HTS NaN 编码、PB7 上拉和长线时序，以及 VBAT ADC/电池百分比阈值校准。
