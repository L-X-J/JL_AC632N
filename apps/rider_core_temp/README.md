...
本轮诊断固件的 Firmware Revision 为 `0.1.6`，Core 模型版本为 `1`。产品固件版本的唯一真相源是 `include/rider_core_temp.h` 中的 `RIDER_CORE_TEMP_FIRMWARE_VERSION`。当前板级设置 `RIDER_UART_HEARTBEAT_ONLY=1`，`app_main()` 只注册一个 2000 ms 串口心跳定时器后返回，不初始化按钮、J12 诊断或 BLE，因此该镜像按设计搜不到 `ICXL-RTemp`。心跳使用底层 `printf()` 绕过 release 模式下可能产生二进制记录的 `log_print()`，串口应收到 `RIDER_HEARTBEAT version=0.1.6` ASCII。串口入口确认后，将该宏改为 `0` 再逐项恢复 BLE；PB3 产品电源键仍由 `RIDER_POWER_KEY_ENABLE=0` 独立关闭。每次人体/骑行实验都应把设备地址、固件版本、模型版本和标定 header 的来源与码表导出文件一起登记；仅凭温度列无法判断数据使用了哪套门控和系数。
