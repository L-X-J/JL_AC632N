...
| PA0 | 旧板 UART0 TX | **悬空，禁止占用** | `TCFG_UART0_TX_PORT = NO_CONFIG_PORT` |
...
| `TCFG_UART0_TX` | PA0 | 产品要求 PA0 悬空，本轮改为 `NO_CONFIG_PORT`；调试需另选空闲 IO |
...
6. **PA0 / USB**：确认固件未再初始化 PA0 为 UART；USB DP/DN 保持关闭配置下的高阻清理，通信功能不受脚位宏改动影响。
