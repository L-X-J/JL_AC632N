...
| PA0 / 调试 UART0 TX | 曾迁 PB5 | **PA0**（排障临时，芯片夹） | `TCFG_UART0_TX_PORT = IO_PORTA_00`；boot `UTTX=PA00` |
...
| `TCFG_UART0_TX` | PB5（过渡） | **排障改回 PA0**（芯片夹）；量产再定是否悬空 |
...
- **调试串口**：TX=`PA0`（115200，boot/应用一致）；当前为排障配置，量产可再悬空。
...
6. **串口 / USB**：USB-TTL RX 接 **PA0**（115200）；USB DP/DN 保持关闭配置下的高阻清理。
