...
// Rider is the only application selected by this board target.
#define CONFIG_APP_RIDER_CORE_TEMP        1 // ICXL CoreTemp Rider BLE 外设
...
/* board_config.h supplies the physical defaults; Rider owns the final
 * product overrides so PB7 cannot be claimed by a demo peripheral and the
 * 1-Wire microsecond timing is never interrupted by low power entry. */
