...
- PB6/PB4、PB0/PB1 的 J12 诊断职责、PB7 的 M601 1-Wire 和 PA0 的 UART0 TX 必须保持不变。Rider 的 boot/OTA 与应用 UART0 调试统一使用 `PA0 / 115200`（8N1）；24 MHz UART 时钟下应用 UART 实际约 `115384 baud`，误差约 `+0.16%`。
- Rider bd19 板级 profile 当前启用 `CONFIG_APP_OTA_ENABLE=1`，在单备份模式下编译 RCSP `AE00/AE01/AE02` BLE OTA 通道；实际升级仍需兼容客户端和硬件验证，微信调试台暂不执行 E1-E7 传输。
