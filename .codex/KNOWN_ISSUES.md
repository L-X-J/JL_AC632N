...
Resolution: The startup gate now requires both the wakeup bit and a live low PB3 level before enforcing the two-second hold. A released PB3 with a stale wakeup bit follows the normal startup prompt. The `0.1.6` isolation image sets `RIDER_UART_HEARTBEAT_ONLY=1`, so it intentionally starts neither BLE nor the PB3 state machine and must only be evaluated by its two-second PA0 heartbeat. Its heartbeat uses direct `printf()` because release `log_print()` records may be binary. Set this macro to `0` before resuming BLE discovery tests; keep `RIDER_POWER_KEY_ENABLE=0` until the application path is stable.
